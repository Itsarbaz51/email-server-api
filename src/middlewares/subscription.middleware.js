...
export const verifySubscription = (action) => {
  return async (req, res, next) => {
    ...
    if (!userId && !mailboxId) {
      return next(new ApiError(401, "Unauthorized: No user or mailbox found"));
    }
    ...
    if (userId) {
      ...
      if (!mailbox) return next(new ApiError(404, "Mailbox not found"));
    ...
    if (!subscription) {
      if (diffDays <= 8) {
        ...
        if (action === "createDomain") {
          ...
          if (count >= 1) return next(new ApiError(403, "Free trial: Only 1 domain allowed"));
        }
        if (action === "createMailbox") {
          const count = await Prisma.mailbox.count({ where: { userId: userId || mailboxId } });
          if (count >= 1) return next(new ApiError(403, "Free trial: Only 1 mailbox allowed"));
        }
        if (action === "sendMail") {
          const count = mailboxId
            ? await Prisma.sentEmail.count({ where: { mailboxId } })
            : await Prisma.sentEmail.count({ where: { userId } });
          if (count >= 50) return next(new ApiError(403, "Free trial: Email send limit exceeded (50)"));
        }
        if (action === "receiveMail") {
          const count = mailboxId
            ? await Prisma.receivedEmail.count({ where: { mailboxId } })
            : await Prisma.receivedEmail.count({ where: { userId } });
          if (count >= 500) return next(new ApiError(403, "Free trial: Email receive limit exceeded (500)"));
        }
        return next();
      }
      return next(new ApiError(403, "Free trial expired. Please subscribe to continue"));
    }
    ...
    if (today > expiryDate) {
      return next(new ApiError(403, "Subscription expired. Please renew to continue"));
    }
    ...
    if (action === "createDomain") {
      const count = await Prisma.domain.count({ where: { userId: userId || mailboxId } });
      if (count >= maxDomains)
        return next(new ApiError(403, `Plan limit exceeded: Max ${maxDomains} domains allowed.`));
    }

    if (action === "createMailbox") {
      const count = await Prisma.mailbox.count({ where: { userId: userId || mailboxId } });
      if (count >= maxMailboxes)
        return next(new ApiError(403, `Plan limit exceeded: Max ${maxMailboxes} mailboxes allowed.`));
    }
    ...
    if (action === "sendMail") {
      ...
      if (count >= maxSentEmails)
        return next(new ApiError(403, `Plan limit exceeded: Max ${maxSentEmails} sent emails allowed.`));
    }
    ...
    if (action === "receiveMail") {
      ...
      if (count >= maxReceivedEmails)
        return next(new ApiError(403, `Plan limit exceeded: Max ${maxReceivedEmails} received emails allowed.`));
    }
